...
- Manual dependency install command.
...
```bash
python3.11 plugins/sap-rpt1/skills/sap-rpt1/scripts/rpt1_oss_predict.py --dry-run --input <csv-path> --target <target-column>
```

- Opt-in local inference command shape:

```bash
python3.11 plugins/sap-rpt1/skills/sap-rpt1/scripts/rpt1_oss_predict.py --run --input <csv-path> --target <target-column> --task classification --output predictions.csv
```
