...
This note records the source basis for the initial `sap-sac-test-automation` skill. The skill distills the local plan into a concise `SKILL.md` plus targeted references for architecture, profile/scenario contracts, agent-browser discovery, Playwright execution, governance/testability, and failure triage.
